...
/** Render the unit body, preserving paragraph breaks, fully HTML-escaped. */
function renderBody(unit: ContentUnit): string {
  const paragraphs = unit.body
    .split(/\n{2,}/)
    .map((p) => p.trim())
    .filter((p) => p !== "");
...
function titleFor(unit: ContentUnit): string {
  const firstLine = unit.body.split("\n", 1)[0]?.trim() ?? "";
  const base = firstLine === "" ? "A witness" : firstLine;
  return base.length > 80 ? base.slice(0, 77) + "..." : base;
}

/** Render a verified ContentUnit to a complete, self-contained HTML page. */
export function renderPage(unit: ContentUnit): string {
  const citations = renderCitations(unit.source_refs);
  return (
    [
      `<!doctype html>`,
      `<html lang="en">`,
      ...
      `  <title>${escapeHtml(titleFor(unit))}</title>`,
